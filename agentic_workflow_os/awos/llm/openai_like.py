"""Драйвер OpenAI-совместимого /v1/chat/completions на стандартной библиотеке.
...
Никаких SDK: `pip install` для ядра среды не требуется, а urllib умеет
...
class OpenAILike(BaseLLM):
    ...
    def __init__(self, model: str, *, base_url: str = "https://api.openai.com/v1",
                 ...
                 temperature: float = 0.2, **kwargs: Any) -> None:
    ...
    def _parse(data: dict[str, Any], model: str) -> Reply:
        ...
        if not text:
            # Reasoning-модели (DeepSeek-R1, Qwen thinking) иногда кладут
            # текст в reasoning_content, оставляя content пустым. Пустой
            # ответ сломал бы шаг на ровном месте — забираем что есть.
...
def _usage(data: dict[str, Any]) -> Usage:
    ...
    if not isinstance(u, dict):
        return Usage()
    tin = u.get("prompt_tokens") or u.get("input_tokens") or u.get(
        "prompt_eval_count") or 0
    tout = u.get("completion_tokens") or u.get("output_tokens") or u.get(
        "eval_count") or 0
    try:
