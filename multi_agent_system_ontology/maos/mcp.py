...
CLIENT_INFO = {"name": "maos", "version": "1.0.0"}
...
class _HttpTransport:
    ...
    def start(self) -> None:
        ...
        # Старый MCP SSE transport: GET /sse держит поток и первым событием
        # `endpoint` сообщает URL, куда затем посылать JSON-RPC POST.
        # Streamable HTTP (/mcp) не требует этого рукопожатия.
        if self.cfg.url.rstrip("/").endswith("/sse"):
            req = urllib.request.Request(self.cfg.url, headers={
                "Accept": "text/event-stream", **self.cfg.headers})
            try:
                self._sse_response = urllib.request.urlopen(req, timeout=self.cfg.timeout)
                event, data = "", ""
                deadline = time.time() + self.cfg.timeout
                while time.time() < deadline:
                    ...
                    if line.startswith("event:"):
                        ...
                        self._endpoint_url = urllib.parse.urljoin(self.cfg.url, data)
                        return
                raise MCPError("SSE-сервер не прислал событие endpoint")
    ...
    def call(self, method: str, params: dict[str, Any] | None,
        ...
        with self.lock:
            ...
            req = urllib.request.Request(
                self._endpoint_url or self.cfg.url,
