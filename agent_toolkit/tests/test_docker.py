...
def run_tests() -> int:
    ...
    check("Dockerfile использует минимальный ubuntu:24.04", "ubuntu:24.04" in df_text)
    ...
    check("Dockerfile устанавливает Chromium и драйверы для браузера", "chromium" in df_text and "chromium-chromedriver" in df_text)
    ...
    check("Dockerfile определяет ENV-переменные для Chromium и Playwright", "CHROME_BIN=/usr/bin/chromium-browser" in df_text and "CHROMEDRIVER_BIN" in df_text)
