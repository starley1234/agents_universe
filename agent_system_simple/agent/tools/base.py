...
def clear_pycache(root: Path, limit: int = 3000) -> int:
    """Убрать __pycache__ внутри папки. Возвращает число удалённых файлов.

    Зачем это вообще нужно. Python решает, устарел ли .pyc, по паре
    «время правки + размер исходника». Агент правит файл и тут же
    прогоняет тест — обе операции в пределах одной секунды. Если правка
    не изменила размер (а замена `w + h` на `w * h` его не меняет),
    кэш считается свежим, и выполняется СТАРЫЙ код.

    Наблюдалось живьём: агент починил функцию, а тест падал с прежней
    ошибкой. Самое неприятное здесь — что виноватым выглядит патч.

    Удалять __pycache__ безопасно: это порождаемый каталог.
    """
    n = 0
    try:
