...
Сначала посмотрите, КТО его занял — сообщение это показывает. Порт 8080
часто держит apache или nginx: их трогать не надо, просто возьмите
другой порт и впишите его в `.env`:
...
Порт 8080 уже занят.
Занят процессом:
  LISTEN 0 5 127.0.0.1:8080 users:(("python3",pid=4757,fd=3))
Что делать:
  · порт держит ЧУЖАЯ программа (см. выше) — её лучше не трогать
  · другой порт:      make serve PORT=8081
  · чтобы не указывать каждый раз, впишите в .env:
                      AGENT_PORT=8081
```

Порт после Ctrl+C освобождается сразу — `allow_reuse_address` избавляет
от минуты ожидания в TIME_WAIT.
