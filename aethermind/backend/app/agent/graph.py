...
class AgentGraph:
    ...
    def _run_llm_step(self, state: dict, step: dict) -> str:
        ...
        prompt = (
            ...
            "Для внешних MCP tools строго соблюдай их input_schema. Если schema требует `code`, передавай OpenSCAD/код в поле `code`, а не `path`; `path` можно указывать только дополнительно. Если есть enum/default — используй допустимые значения. "
