...
class AgentGraph:
    ...
    def _run_llm_step(self, state: dict, step: dict) -> str:
        ...
        prompt = (
            ...
            "Для внешних MCP tools строго соблюдай их input_schema. Если schema требует `code`, передавай OpenSCAD/код в поле `code`, а не `path`; `path` можно указывать только дополнительно. Если есть enum/default — используй допустимые значения. Для OpenSCAD предпочитай openscad.render_2d_png для быстрых итераций или openscad.generate_and_analyze для проверки ошибок, а не generic render. "
