...
- переключение просмотра артефакта `Markdown` / `HTML`;
- прикрепление изображений к контексту задачи через кнопку `📎 Прикрепить изображение`;
...
- наглядную Human-in-the-loop панель: причина критика, confidence, observation, последние проблемные события и быстрые варианты ответа; вариант «принять риск» теперь действительно помечает текущий шаг выполненным и не дает критику повторно блокировать тот же шаг;
